...
import { cortesDetallePorSlug, type CorteDetalle } from './cortesDetalle'
...
export interface CorteArea {
  ...
  image?: string
  /** Slug en cortesDetalle con la foto y los datos del corte. */
  detalle?: string
  /**
   * Ajuste del rótulo sobre el mapa. Por defecto se centra en el centroide del
   * polígono; dx/dy lo corren en % y rotate lo gira (90 = vertical, para las
   * zonas estrechas del brazo y la pierna).
   */
  label?: { dx?: number; dy?: number; rotate?: number; scale?: number }
}

/** Placeholder hasta reemplazar por foto real de cada corte. */
export const CORTE_FICHA_EJEMPLO = images.corteFichaEjemplo

export function getCorteDetalle(area: CorteArea): CorteDetalle | null {
  return (area.detalle && cortesDetallePorSlug[area.detalle]) || null
}

export function getCorteFichaImage(area: CorteArea) {
  const imagen = area.image ?? getCorteDetalle(area)?.imagen

  return imagen ? assetUrl(imagen) : CORTE_FICHA_EJEMPLO
}

function parsePolygonPoints(clipPath: string): Array<[number, number]> {
  const inner = clipPath.slice(clipPath.indexOf('(') + 1, clipPath.lastIndexOf(')'))
...
/** Puntos del polígono para un <polygon> con viewBox 0 0 100 100. */
export function getAreaPoints(area: CorteArea) {
  return parsePolygonPoints(area.clipPath)
    .map(([x, y]) => `${x},${y}`)
    .join(' ')
}

/** Centroide del polígono, en % sobre la imagen, para colocar el rótulo del corte. */
export function getAreaCenter(area: CorteArea) {
  const points = parsePolygonPoints(area.clipPath)
  let signedArea = 0
  let cx = 0
  let cy = 0
...
export const cortesMapAreas: CorteArea[] = [
  {
    ...
    slug: 'punta-de-anca',
    name: 'Punta de anca',
    detalle: 'punta-de-anca',
    label: { dx: 1.5, dy: 1, scale: 0.85 },
    grupo: 'traseros',
    clipPath:
  ...
  },
  {
    ...
    detalle: 'centro-de-pierna',
    label: { dx: 0.3, dy: 3, rotate: 90, scale: 0.75 },
    grupo: 'traseros',
    clipPath:
  ...
  },
  {
    ...
    name: 'Muchacho',
    grupo: 'traseros',
    label: { dy: 2, rotate: 90, scale: 0.8 },
    clipPath:
  ...
  },
  {
    ...
    slug: 'bota',
    name: 'Bota',
    detalle: 'posta-bota-de-herradero',
    label: { dx: -0.5, dy: 2, rotate: 90, scale: 0.8 },
    grupo: 'traseros',
    clipPath:
  ...
  },
  {
    ...
    grupo: 'traseros',
    label: { dx: 1, rotate: 90, scale: 0.7 },
    clipPath:
  ...
  },
  {
    ...
    slug: 'bola-de-pierna',
    name: 'Bola de pierna',
    detalle: 'bola-de-pierna',
    label: { dx: -1, dy: 1.5, scale: 0.9 },
    grupo: 'traseros',
    clipPath:
  ...
  },
  {
    ...
    slug: 'bola-de-brazo',
    name: 'Bola de brazo',
    detalle: 'bola-de-brazo',
    label: { rotate: 90, scale: 0.7 },
    grupo: 'delanteros',
    clipPath:
  ...
  },
  {
    ...
    detalle: 'paletero',
    label: { dx: 0.5, rotate: 90, scale: 0.62 },
    grupo: 'delanteros',
    clipPath:
...
/** Una zona por nombre: 'Cogote' ocupa dos polígonos superpuestos. */
export const cortesMapZonas = cortesMapAreas.filter(
  (area, i) => cortesMapAreas.findIndex((a) => a.name === area.name) === i,
)
